"""
scripts/run_p2p2c_lod_ladder.py
...
learned channel. Run: python -u scripts/run_p2p2c_lod_ladder.py
"""
...
def _run_arm(name, make_source, tr, va, te, cfg, git, seed):
    ...
    out = train_delta_prior(make_source(tr), make_source(va), make_source(te), cfg,
                            RNGState(seed=seed), kind="main", run_id=f"p2p2c-ladder-{name}",
    ...
    write_manifest(Path(f"runs/p2p2c-ladder-{name}.json"), out["manifest"])
    return row
...
def main():
    ...
    print("=" * 96)
    print("P2.2c LOD MODEL LADDER — held-out A/B (own-value vs LOD), leave-datasets-out, target-conditioned")
    ...
    Path("runs/p2p2c-ladder-summary.json").write_text(json.dumps({"git": git, "rows": rows, "verdict": verdict}, indent=2))
    print("\nsaved: runs/p2p2c-ladder-{ownvalue,lod}.json + summary")
