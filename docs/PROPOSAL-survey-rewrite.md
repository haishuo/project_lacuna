...
buildable before building it).

- **P0 — paper resolutions (no code):** settle §5.1 (δ coordinate system) and the B2 validity
  envelope; define the footprint vector and the survey-idiom list.
- **P1 — decisive feasibility (small code, mostly reuse):** (a) **oracle distinguishability** at matched
  rate per idiom (reuse generators + footprint stats; no network); (b) **leave-one-form-out** within an
  axis. Gate: if oracle says no signal exists, stop and report — honestly and early.
