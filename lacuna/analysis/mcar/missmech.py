...
def _between_pattern_ss(
    ...
    Kept for the observed statistic (called once). The permutation loop
    uses the faster ``_permuted_between_pattern_ss_vec``, which avoids
    this function's Python-level per-pattern loop.
    """
...
def _permutation_null_vec(
    X_keep: np.ndarray, n_per_pattern: np.ndarray, n_permutations: int,
    seed: int,
) -> np.ndarray:
    """Fully vectorised permutation null for the between-pattern SS.

    Inputs
    ------
    X_keep : (n_keep, d) float
        Imputed rows belonging to a kept pattern; label order is arbitrary
        (we're going to shuffle it regardless).
    n_per_pattern : (n_kept_patterns,) int
        Pattern sizes. Must sum to n_keep.
    n_permutations : int
    seed : int

    Returns
    -------
    (n_permutations,) float
        Between-pattern SS statistic under each permutation.

    Algebraic speedup
    -----------------

    Let μ be the grand mean (invariant under label permutation). Then
        Σ_p n_p ||μ_p − μ||²
      = Σ_p n_p ||μ_p||² − n ||μ||²
      = Σ_p ||S_p||² / n_p  − n ||μ||²

    where S_p is the sum of X rows carrying label p. Under permutation
    only S_p changes, so we only need per-permutation scatter-sums of
    ``X_keep`` into (n_kept_patterns, d) bins. The rest is a
    deterministic quadratic form.

    Runtime is O(n_perm · n_keep · d) with a Python loop only over
    permutations (the scatter itself is a single numpy call).
    """
    rng = np.random.default_rng(seed)
    n_keep, d = X_keep.shape
    P = n_per_pattern.size
    grand_mean = X_keep.mean(axis=0)
    n_per_pattern = n_per_pattern.astype(np.int64)
    # Invariant constant n ||μ||².
    invariant = float(n_keep * np.dot(grand_mean, grand_mean))

    # Fixed label vector: block-constant, pattern p appears n_per_pattern[p] times.
    # Permuting this vector is equivalent to permuting WHICH row carries which
    # label, which is the correct null for "label independent of X".
    fixed_labels = np.repeat(np.arange(P, dtype=np.int64), n_per_pattern)

    out = np.empty(n_permutations, dtype=np.float64)
    for k in range(n_permutations):
        perm = rng.permutation(n_keep)
        perm_labels = fixed_labels[perm]
        # Scatter-add: S[p] = Σ_{i : perm_labels[i] == p} X_keep[i]
        S = np.zeros((P, d), dtype=np.float64)
        np.add.at(S, perm_labels, X_keep)
        # Σ_p ||S_p||² / n_p
        sq_norms = np.einsum("pd,pd->p", S, S)
        stat = float(np.sum(sq_norms / n_per_pattern) - invariant)
        out[k] = stat
    return out
...
def missmech_mcar_test(
    ...
    *,
    ...
    # Permutation null via vectorised scatter-add. Algebraically
    # equivalent to the per-permutation ``_between_pattern_ss`` loop but
    # with the per-pattern Python loop removed.
    keep_idx = np.where(np.isin(labels, kept_patterns))[0]
    X_keep = X_imputed[keep_idx]
    n_per_pattern = np.array(
        [int((labels[keep_idx] == p).sum()) for p in kept_patterns],
        dtype=np.int64,
    )
    null_stats = _permutation_null_vec(
        X_keep, n_per_pattern, n_permutations, seed,
    )
    null_sum = float(null_stats.sum())
    n_ge = int(np.sum(null_stats >= observed_stat))
