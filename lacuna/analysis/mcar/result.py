"""Shared result dataclass for distribution-free MCAR tests.

Originally lived at ``pystatistics.nonparametric_mcar.result``; moved
to Lacuna on 2026-04-20 as part of the scope-boundary cleanup (see
Coding Bible rule 8). These tests were motivated by Lacuna's
mcar-alternatives-bakeoff and are not general-purpose statistical
tools, so they belong in Lacuna. pystatistics keeps only the textbook
Little (1988) MLE-plug-in test.
"""
...
class NonparametricMCARResult:
    ...
    statistic : float
        Test statistic. Interpretation depends on ``method``:
          - propensity: pooled out-of-fold AUC minus 0.5, clipped to
            [0, 0.5]. 0 = indistinguishable from chance (consistent
            with MCAR); 0.5 = perfect separation.
          - HSIC: Hilbert-Schmidt independence criterion estimate.
          - MissMech: between-pattern weighted sum-of-squared-mean-
            differences after k-NN imputation.
    p_value : float
