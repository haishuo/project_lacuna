...
def test_hsic_on_mcar_data_does_not_reject():
    ...
    r = hsic_mcar_test(X, seed=0)  # default null='gamma'
    assert isinstance(r, NonparametricMCARResult)
    assert "HSIC" in r.method
    assert "gamma" in r.method
...
def test_hsic_gamma_and_permutation_agree_under_mcar():
    """Both null modes should give similar (non-rejecting) p-values on
    MCAR data. Exact numeric agreement isn't expected — permutation has
    Monte-Carlo noise of ~0.05 at n_perm=199, and the gamma
    approximation has its own bias. ±0.25 is a generous but meaningful
    band that catches pathological divergence."""
    X = _mcar_matrix(n=120, d=4, p_miss=0.2, seed=2)
    r_gamma = hsic_mcar_test(X, null="gamma", seed=0)
    r_perm = hsic_mcar_test(X, null="permutation", n_permutations=199, seed=0)
    # Same statistic, different p-value.
    assert abs(r_gamma.statistic - r_perm.statistic) < 1e-12
    assert abs(r_gamma.p_value - r_perm.p_value) < 0.25


def test_hsic_gamma_and_permutation_agree_under_mar():
    """Both null modes should reject on strong MAR data. Here we just
    assert both give very small p-values (< 0.1)."""
    X = _mar_matrix(n=200, d=4, seed=2)
    r_gamma = hsic_mcar_test(X, null="gamma", seed=0)
    r_perm = hsic_mcar_test(X, null="permutation", n_permutations=199, seed=0)
    assert r_gamma.p_value < 0.1
    assert r_perm.p_value < 0.1


def test_hsic_permutation_mode_runs():
    X = _mcar_matrix(n=60, d=4, seed=0)
    r = hsic_mcar_test(X, null="permutation", n_permutations=49, seed=0)
    assert "perm=49" in r.method
    assert "permutation_null_mean_hsic" in r.extra


def test_hsic_reproducible_same_seed():
    X = _mcar_matrix(n=60, d=4, seed=1)
    r1 = hsic_mcar_test(X, seed=42)
    r2 = hsic_mcar_test(X, seed=42)
...
def test_raises_on_invalid_alpha():
    ...
    with pytest.raises(ValueError, match="alpha"):
        hsic_mcar_test(X, alpha=0.0)


def test_raises_on_invalid_null():
    X = _mcar_matrix(n=30, d=3, seed=0)
    with pytest.raises(ValueError, match="null must be"):
        hsic_mcar_test(X, null="bayesian")
