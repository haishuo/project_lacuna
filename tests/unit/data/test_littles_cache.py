...
def test_compute_entry_populates_fields(small_raw):
    gen = _BernoulliMCAR()
    entry = compute_entry(small_raw, gen, rng=RNGState(seed=1), sample_rows=100)
...
def test_cache_get_returns_pair_for_both_methods(small_raw):
    gen = _BernoulliMCAR()
    cache = build_cache(
        raw_datasets=[small_raw],
        generators=[gen],
    ...
    mle_stat, mle_p = cache.get("small", 0, method="mle")
    mom_stat, mom_p = cache.get("small", 0, method="mom")
    assert mle_stat == entry.mle_statistic
    assert mle_p == entry.mle_p_value
    assert mom_stat == entry.mom_statistic
    assert mom_p == entry.mom_p_value
...
def test_no_missingness_produces_degenerate_entry(small_raw):
    """A generator that never drops values -> stat=0, p=1 sentinel (both methods)."""
    gen = _NoMissing()
...
def test_loader_validates_cache_coverage(small_raw):
    ...
    partial = build_cache(
        raw_datasets=[small_raw],
        generators=[gen_a],
        generator_registry_name="partial",
        sample_rows=100,
    )
